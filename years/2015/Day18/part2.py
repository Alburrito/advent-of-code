...
class Neighborhood:
    ...
    def __init__(self, data: List[str]):
        ...
        # Corners ON
        self.neighborhood[0] = ON + self.neighborhood[0][1:-1] + ON
        self.neighborhood[len(self.neighborhood) - 1] = ON + self.neighborhood[len(self.neighborhood) - 1][1:-1] + ON
    ...
    def _step(self):
        ...
        for i, street in enumerate(self.neighborhood):
            ...
            for j, _ in enumerate(street):
                    if self._is_corner(i,j):
                        new_light_state = self.neighborhood[i][j]
                    else:
                        new_light_state = self._next_light_state(i, j)
                    new_state[i] += new_light_state
        self.neighborhood = new_state

    def _is_corner(self, i: int, j: int) -> bool:
        """
        Determines if a light at a given position is a corner of the grid.
        ...
        Returns:
            bool: True if the light is a corner, False otherwise.
        """
        if i in (0, len(self.neighborhood[0]) - 1) \
        and j in (0, len(self.neighborhood) -1):
            return True
        return False
